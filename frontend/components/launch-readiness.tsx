...
const labels: Record<string, { ar: string; en: string }> = {
  ...
  shipping: { ar: "مناطق وأسعار الشحن", en: "Shipping rates" },
  email: { ar: "البريد الإلكتروني", en: "Email delivery" },
  payment: { ar: "الدفع الإلكتروني", en: "Online payments" },
...
export function LaunchReadiness({ locale }: { locale: Locale }) {
  ...
  return <main className="content-page shell commerce-page"><p className="eyebrow">XVOND STORE ADMIN</p><h1>{ar ? "جاهزية إطلاق المتجر" : "Store launch readiness"}</h1><p>{ar ? "فحص مباشر للإعدادات الأساسية المطلوبة قبل استقبال طلبات حقيقية." : "Live checks for the core configuration required before accepting real orders."}</p><div className="admin-actions"><Link className="secondary-button" href={`/${locale}/admin`}>{ar ? "لوحة الإدارة" : "Admin dashboard"}</Link><Link className="secondary-button" href={`/${locale}/admin/shipping`}>{ar ? "إدارة الشحن" : "Shipping management"}</Link></div>{error && <p className="form-error">{error}</p>}{!data && !error && <p>{ar ? "جارٍ الفحص…" : "Checking…"}</p>}{data && <><div className="pending-choice"><strong>{data.ready ? (ar ? "المتجر جاهز من ناحية الإعدادات التقنية" : "Technical launch checks are ready") : (ar ? "يوجد إعدادات ناقصة قبل الإطلاق" : "Some launch configuration is still missing")}</strong><p>{data.ready_count}/{data.total_checks} {ar ? "بنود جاهزة" : "checks ready"}</p></div><div className="admin-cards">{data.checks.map((check) => <article key={check.key}><div><strong>{labels[check.key]?.[ar ? "ar" : "en"] || check.key}</strong><small>{check.detail}</small></div><strong>{check.ready ? "✓" : "—"}</strong></article>)}</div></>}</main>;
}
