...
export function CartView({ locale }: { locale: Locale }) {
  ...
  return (
    <main className="content-page shell commerce-page">
      <p className="eyebrow">XVOND STORE</p><h1>{ar ? "سلة التسوق" : "Shopping cart"}</h1>
      {cart.length === 0 ? (
        <div className="empty-card"><p>{ar ? "سلتك فارغة حاليًا." : "Your cart is currently empty."}</p><Link className="primary-button" href={`/${locale}`}>{ar ? "متابعة التسوق" : "Continue shopping"}</Link></div>
      ) : (
          ...
          <div className="cart-lines">
            {cart.map(({ product, quantity }) => (
              <article className="cart-line" key={product.slug}>
                ...
                <div className="cart-line-copy"><Link href={`/${locale}/product/${product.slug}`}>{product.name[locale]}</Link><strong>{formatPrice(product.price, locale)}</strong></div>
